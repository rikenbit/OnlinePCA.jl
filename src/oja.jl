...
"""
function oja(;input::AbstractString="", outdir::Union{Void,AbstractString}=nothing, scale::AbstractString="ftt", pseudocount::Number=1.0, rowmeanlist::AbstractString="", rowvarlist::AbstractString="", colsumlist::AbstractString="", masklist::AbstractString="", dim::Number=3, stepsize::Number=0.1, numepoch::Number=5, scheduling::AbstractString="robbins-monro", g::Number=0.9, epsilon::Number=1.0e-8, logdir::Union{Void,AbstractString}=nothing)
    # Initial Setting
    pca = OJA()
    ...
    # Perform PCA
    out = oja(input, outdir, scale, pseudocount, rowmeanlist, rowvarlist, colsumlist, masklist, dim, stepsize, numepoch, scheduling, g, epsilon, logdir, pca, W, v, D, rowmeanvec, rowvarvec, colsumvec, maskvec, N, M, AllVar)
...
function oja(input, outdir, scale, pseudocount, rowmeanlist, rowvarlist, colsumlist, masklist, dim, stepsize, numepoch, scheduling, g, epsilon, logdir, pca, W, v, D, rowmeanvec, rowvarvec, colsumvec, maskvec, N, M, AllVar)
    N, M = nm(input)
    tmpN = zeros(UInt32, 1)
    tmpM = zeros(UInt32, 1)
    x = zeros(UInt32, M)
    ...
    for s = 1:numepoch
        open(input) do file
            ...
            for n = 1:N
                ...
                # Update Eigen vector
                @show scheduling, stepsize, g, epsilon, D, N, M, W[1:2,1:2], v[1], normx[1:2], s, n
                if n == 10
                    break
                end
...
# Oja × Robbins-Monro
function ojaupdate(scheduling::ROBBINS_MONRO, stepsize, g, epsilon, D, N, M, W, v, normx, s, n)
    W .= W .+ ∇fn(W, normx, D , M, stepsize/(N*(s-1)+n))
    v = nothing
    return W, v
end

# Oja × Momentum
function ojaupdate(scheduling::MOMENTUM, stepsize, g, epsilon, D, N, M, W, v, normx, s, n)
    v .= g .* v .+ ∇fn(W, normx, D, M, stepsize)
    W .= W .+ v
    return W, v
end

# Oja × NAG
function ojaupdate(scheduling::NAG, stepsize, g, epsilon, D, N, M, W, v, normx, s, n)
    v = g .* v + ∇fn(W - g .* v, normx, D, M, stepsize)
    W .= W .+ v
    return W, v
end

# Oja × Adagrad
function ojaupdate(scheduling::ADAGRAD, stepsize, g, epsilon, D, N, M, W, v, normx, s, n)
    grad = ∇fn(W, normx, D, M, stepsize)
    grad = grad / stepsize
    v .= v .+ grad .* grad
