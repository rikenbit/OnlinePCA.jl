...
function svrg(;input="", output=".", dim=3, stepsize=0.1, numepoch=5, scheduling="robbins-monro", g=0.9, epsilon=0.00000001, logfile=false)
        ...
        open(input) do file
            ...
            for n = 1:N
                ...
                if scheduling == "robbins-monro"
                    # SVRG × Robbins-Monro
                    W .= W .+ stepsize/(N*(s-1)+n) .* (∇fn(W, x, D) .- ∇fn(Ws, x, D) .+ u)
                elseif scheduling == "momentum"
                    # SVRG × Momentum
                    v .= g .* v .+ Float32(stepsize) .* (∇fn(W, x, D) .- ∇fn(Ws, x, D) .+ u)
                    W .= W .+ v
                elseif scheduling == "nam"
                    # SVRG × NAM
                    v = g .* v + Float32(stepsize) .* (∇fn(W - g .* v, x, D) .- ∇fn(Ws, x, D) .+ u)
                    W .= W .+ v
                elseif scheduling == "adagrad"
                    # SVRG × Adagrad
                    grad = (∇fn(W, x, D) .- ∇fn(Ws, x, D) .+ u)
                    v .= v .+ grad .* grad
...
function main()
    ...
    out = svrg(input=parsed_args["input"],
...
# # test
# @time out1 = svrg(input="test.dat", dim=3, stepsize=0.1, numepoch=5)
# @time out2 = svrg(input="test2.dat", dim=3, stepsize=0.1, numepoch=5)
# @time out3 = svrg(input="test3.dat", dim=3, stepsize=0.1, numepoch=5)
# @time out4 = svrg(input="test3.dat", dim=30, stepsize=0.1, numepoch=5)
...
# abs(cor(Cvec[1:10,:]', svrg(input="test2.dat", dim=10, stepsize=0.1, numepoch=5)[1]))
