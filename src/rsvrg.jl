...
function rsvrg(;input::AbstractString="", outdir::Union{Nothing,AbstractString}=nothing, scale::AbstractString="ftt", pseudocount::Number=1f0, rowmeanlist::AbstractString="", rowvarlist::AbstractString="",colsumlist::AbstractString="", dim::Number=3, stepsize::Number=0.1, numbatch::Number=100, numepoch::Number=3, scheduling::AbstractString="robbins-monro", g::Number=0.9, epsilon::Number=1.0e-8, lower::Number=0, upper::Number=1.0f+38, expvar::Number=0.1f0, evalfreq::Number=5000, offsetFull::Number=1f-20, offsetStoch::Number=1f-6, initW::Union{Nothing,AbstractString}=nothing, initV::Union{Nothing,AbstractString}=nothing, logdir::Union{Nothing,AbstractString}=nothing, perm::Bool=false, cper::Number=1f0)
    ...
    # Perform PCA
    out = rsvrg(input, outdir, scale, pseudocount, rowmeanlist, rowvarlist, colsumlist, dim, stepsize, numbatch, numepoch, scheduling, g, epsilon, logdir, pca, W, v, D, rowmeanvec, rowvarvec, colsumvec, N, M, TotalVar, lower, upper, evalfreq, offsetFull, offsetStoch, perm, cper)
...
function rsvrg(input, outdir, scale, pseudocount, rowmeanlist, rowvarlist, colsumlist, dim, stepsize, numbatch, numepoch, scheduling, g, epsilon, logdir, pca, W, v, D, rowmeanvec, rowvarvec, colsumvec, N, M, TotalVar, lower, upper, evalfreq, offsetFull, offsetStoch, perm, cper)
    N, M = nm(input)
    tmpN = zeros(UInt32, 1)
    tmpM = zeros(UInt32, 1)
    x = zeros(UInt32, M)
    ...
    while(stop == 0 && s <= numepoch)
        ...
        open(input) do file
            ...
            while(stop == 0 && n <= N)
                ...
                # Update Eigen vector
                W, v, batchgrad1, batchgrad2, batchidx, batchcount = rsvrgupdate(scheduling, stepsize, g, epsilon, D, N, M, dim, W, v, batchgrad1, batchgrad2, batchidx, batchcount, numbatch, normx, s, n, u, Ws, offsetStoch)
                # NaN
...
# RSVRG × Robbins-Monro
function rsvrgupdate(scheduling::ROBBINS_MONRO, stepsize, g, epsilon, D, N, M, dim, W, v, batchgrad1, batchgrad2, batchidx, batchcount, numbatch, normx, s, n, u, Ws, offsetStoch)
    if mod(batchidx, numbatch) == 0 && n != 1
        W .= W .+ Pw(batchgrad1, W) .- Pw(batchgrad2, Ws) .+ u
        batchgrad1 = zeros(Float32, M, dim)
        batchgrad2 = zeros(Float32, M, dim)
        batchidx = 1
        batchcount += 1
...
# RSVRG × Momentum
function rsvrgupdate(scheduling::MOMENTUM, stepsize, g, epsilon, D, N, M, dim, W, v, batchgrad1, batchgrad2, batchidx, batchcount, numbatch, normx, s, n, u, Ws, offsetStoch)
    if mod(batchidx, numbatch) == 0 && n != 1
        v .= g .* v .+ Pw(batchgrad1, W) .- Pw(batchgrad2, Ws) .+ u
        W .= W .+ v
        batchgrad1 = zeros(Float32, M, dim)
        batchgrad2 = zeros(Float32, M, dim)
        batchidx = 1
...
# RSVRG × NAG
function rsvrgupdate(scheduling::NAG, stepsize, g, epsilon, D, N, M, dim, W, v, batchgrad1, batchgrad2, batchidx, batchcount, numbatch, normx, s, n, u, Ws, offsetStoch)
    if mod(batchidx, numbatch) == 0 && n != 1
        v = g .* v + Pw(batchgrad1, W - g .* v) .- Pw(batchgrad2, Ws) .+ u
        W .= W .+ v
        batchgrad1 = zeros(Float32, M, dim)
        batchgrad2 = zeros(Float32, M, dim)
        batchidx = 1
...
# RSVRG × Adagrad
function rsvrgupdate(scheduling::ADAGRAD, stepsize, g, epsilon, D, N, M, dim, W, v, batchgrad1, batchgrad2, batchidx, batchcount, numbatch, normx, s, n, u, Ws, offsetStoch)
    if mod(batchidx, numbatch) == 0 && n != 1
        grad = Pw(batchgrad1, W) .- Pw(batchgrad2, Ws) .+ u
        grad = grad / stepsize
        v .= v .+ grad .* grad
