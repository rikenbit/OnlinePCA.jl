...
- **Step.2 Summarization** : `sumr` function parses the binary file and then extract some summary statistics in each row and each column. The information is used by Step.3. Assuming a situation where the input file is the HDF5 formatted by 10X Genomics, we also prepared `tenxsumr` function.
